package com.example.solicare.aichat.service;
...
import com.example.solicare.aichat.repository.ChatMessageRepository;
...
public class AiChatService {
    ...
    public Mono<ChatResponseDto> chat(ChatRequestDto req) {
        // 1. DB에서 기존 대화 내역 조회
        List<ChatMessage> dbHistory = chatMessageRepository.findBySessionIdOrderByCreatedAtAsc(req.sessionId());
        ...
        List<ChatMessageDto> history = dbHistory.stream()
        ...
        return aiService.askChatGPTWithHistory(payloadMessages)
                .flatMap(answer -> {
                    ...
                    return Mono.just(new ChatResponseDto(req.sessionId(), finalHistory, answer));
                });
    }
